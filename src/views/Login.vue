  ...
  <div>
    <div class="max-w-md mx-auto mt-8 p-5 bg-white rounded">
      ...
      <p v-if="hasError" class="errors">
        Login fehlgeschlagen. Bitte überprüfen Sie E-Mail und Passwort oder
        fahren Sie als Gast fort.
      </p>
      <p><input type="text" placeholder="E-Mail" v-model="email" /></p>
      <p><input type="password" placeholder="Password" v-model="password" /></p>
      <button @click="login" class="button w-full">Einloggen</button>
    </div>

    <div class="max-w-md mx-auto mt-8 p-5 bg-white rounded">
      <div class="text-2xl mb-4">Keine Zugangsdaten?</div>
      <p>
        Interessierte Besucher können sich das Projekt über den Gastzugang
        ansehen.
      </p>
      <button @click="loginAsGuest" class="button w-full">
        Als Gast fortfahren
      </button>
    </div>
  </div>
